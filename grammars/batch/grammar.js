...
module.exports = grammar({
  ...
  extras: () => [/[ \t]/],
  word: ($) => $.command_name,
  conflicts: ($) => [[$.parenthesized, $.paren_expression]],
  rules: {
              ...
              choice(
                seq(
                  ...
                  repeat1(
                    choice(
                      $.variable_reference,
                      alias($._quoted_var_name_pattern, $.variable_name),
                    ),
                  ),
                  optional(seq("=", optional($.quoted_assignment_value))),
                  '"',
                ...
                seq(
                  ...
                  optional(seq("=", optional($.caret_quoted_assignment_value))),
                ...
                seq(
                  repeat1(
                    choice(
                      $.variable_reference,
                      alias($._var_name_pattern, $.variable_name),
                    ),
                  ),
                  optional(seq("=", optional($.assignment_value))),
                ),
              ),
            ),
          ),
    ...
    _quoted_var_name_pattern: () =>
      token(prec(1, choice(/[^\s="%][^="%]*/, /%%[a-zA-Z]?/, /"[^="\r\n]+"/))),
    arithmetic_assignment: ($) =>
      seq(
        optional(/[ \t]+/),
        alias(ci("/a"), $.set_option),
        optional(/[ \t]+/),
    ...
    prompt_assignment: ($) =>
      seq(
        optional(/[ \t]+/),
        alias(ci("/p"), $.set_option),
        optional(/[ \t]+/),
        choice(
          seq(
            optional(alias($._var_name_pattern, $.variable_name)),
            "=",
            optional($.assignment_value),
          ),
          seq(
            '"',
            optional(alias($._var_name_pattern, $.variable_name)),
            "=",
            optional($.quoted_assignment_value),
            '"',
            optional($.argument_list),
          ),
        ),
      ...
      prec.right(
        ...
        seq(
          ...
          choice(
            ...
            seq(
              kw("defined"),
              choice(/[$a-zA-Z_][$a-zA-Z0-9_.]*/, $._if_operand),
            ),
            seq(kw("errorlevel"), $.integer),
            seq($._if_operand, $.comparison_op, $._if_operand),
          ),
          choice(
    ...
    else_clause: ($) =>
      prec.right(8, seq(kw("else"), choice($.parenthesized, $._body_stmt))),
    _body_stmt: ($) =>
      choice(
        $.cmd,
        $.variable_assignment,
    ...
    setlocal_stmt: () =>
      prec(
        ...
        seq(
          ...
          repeat(
            choice(
              kw("enabledelayedexpansion"),
              kw("disabledelayedexpansion"),
              kw("enableextensions"),
              kw("disableextensions"),
            ),
          ),
        ),
      ),
    ...
    for_options: () =>
      token(
        prec(
          ...
          choice(
            ci("/d"),
            seq(ci("/r"), optional(seq(/[ \t]+/, /(%[^\s%]|[^\s%])+%?/))),
            ci("/l"),
            seq(ci("/f"), optional(seq(/[ \t]+/, '"', /[^"]*/, '"'))),
          ),
        ),
      ),
    ...
    redirect_stmt: ($) =>
      prec.right(
        ...
        choice(
          seq(
            choice($.call_stmt, $.cmd, $.parenthesized, $.variable_assignment),
            $.redirection,
          ),
          seq(
            $.redirection,
    ...
    fd_redirect: ($) =>
      choice(
        ...
        seq(
          alias(token(seq(optional(/[0-9]/), />&|<&/)), $.fd_redirect_op),
          $.variable_reference,
        ),
      ),
    redirect_op: () => token(choice(/[0-9]?>>/, /[0-9]?>/, /[0-9]?</)),
    redirect_target: () =>
      token(choice(ci("nul"), ci("con"), /[^\s|&><\r\n]+/)),
    pipe_stmt: ($) =>
      prec.left(
        3,
        seq(
          ...
          "|",
          choice($.redirect_stmt, $.call_stmt, $.cmd, $.parenthesized),
        ),
      ),
    cond_exec: ($) =>
      choice(
    ...
    cmd: ($) =>
      prec.right(
        ...
        choice(
          seq(
            optional("@"),
            alias(kw("echo"), $.command_name),
            optional(alias($._echo_text, $.argument_list)),
          ),
          seq(
            optional("@"),
            choice($.command_name, $.variable_reference, $.string),
            optional($.argument_list),
          ),
        ),
      ),
    ...
    macro_invocation: ($) =>
      prec.right(
        6,
        seq($.variable_reference, $.parenthesized, optional($.else_clause)),
      ),
